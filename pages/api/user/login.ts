    ...
    if (req.method === 'POST') {
        ...
        if (user === null) {
            ...
            if (!result) {
                ...
                const token: string = createToken(user.email, user.name, user.id)
                nookies.set({ res }, 'receitinha-token', token, {
                    maxAge: 60 * 60 * 16,
                    path: '/',
                })
                const response: IResponse = { error: false, msg: "Sucesso", data: { id: user.id, name: user.name, email: user.email } }
